...
class EmployeeDeduction(Base):
    ...
    deduction_type_id = Column(Integer, ForeignKey("deduction_types.id", ondelete="CASCADE"), nullable=False)
    
    # Relationships
    owner = relationship("Employee", primaryjoin="EmployeeDeduction.emp_id == Employee.emp_id", foreign_keys=[emp_id], backref=backref("deductions", cascade="all, delete-orphan"))
