...
class AttendanceCorrectionRequest(BaseModel):
    ...
    def validate_status(cls, v: str) -> str:
        ...
        valid_statuses = ['PRESENT', 'ABSENT', 'ON_LEAVE']
...
class SalaryStructureCreate(BaseModel):
    """Salary structure creation validation"""
    emp_id: str
    basic_salary: float = Field(..., gt=0)
    hra: Optional[float] = Field(default=0, ge=0)
    transport_allowance: Optional[float] = Field(default=0, ge=0)
    dearness_allowance: Optional[float] = Field(default=0, ge=0)
    medical_allowance: Optional[float] = Field(default=0, ge=0)
    special_allowance: Optional[float] = Field(default=0, ge=0)
    other_allowances: Optional[float] = Field(default=0, ge=0)
...
class DeductionTypeCreate(BaseModel):
    """Deduction type creation validation"""
    name: str = Field(..., min_length=1, max_length=100)
    description: Optional[str] = Field(None, max_length=500)
    calculation_type: str = Field(..., description="FIXED or PERCENTAGE")
    default_value: Optional[float] = Field(None, ge=0)
    is_mandatory: bool = False
    
    @field_validator('calculation_type')
    @classmethod
    def validate_calculation_type(cls, v: str) -> str:
        if v.upper() not in ['FIXED', 'PERCENTAGE']:
            raise ValueError('Calculation type must be FIXED or PERCENTAGE')
        return v.upper()


class EmployeeDeductionCreate(BaseModel):
    """Employee deduction creation validation"""
    emp_id: str
    deduction_type_id: int = Field(..., gt=0)
    calculation_type: str = Field(..., description="FIXED or PERCENTAGE")
...
class PayrollProcessRequest(BaseModel):
    """Payroll processing request validation"""
    emp_id: str
